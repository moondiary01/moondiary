...
Page({
  ...
  checkAndLoad: function () {
    ...
    /* 试用到期且未购买：每次进主页都弹购买页，功能锁定 */
    if (!app.canUse()) {
      this.setData({ showPayModal: true, isLocked: true, payModalMode: 'expired' })
      return
    }
    /* 正常使用：如果弹窗是 unlock 模式（用户正在看购买信息），保持不关闭 */
    if (this.data.payModalMode === 'unlock' && this.data.showPayModal) {
      this.loadUserData()
      return
    }
    /* 关闭弹窗，解除锁定，加载数据 */
    this.setData({ showPayModal: false, isLocked: false, payModalMode: 'expired' })
    this.loadUserData()
  },

  onPayModalClose: function () {
    /* 主动购买模式（unlock）：关闭后正常使用，不锁定 */
    if (this.data.payModalMode === 'unlock') {
      this.setData({ showPayModal: false })
      return
    }
    /* 试用到期模式（expired）：如果已购买/激活成功则解锁，否则保持锁定 */
    if (app.canUse()) {
      this.setData({ showPayModal: false, isLocked: false })
      this.loadUserData()
    } else {
      this.setData({ showPayModal: false, isLocked: true })
    }
  },
  ...
  initPage: function (state, today) {
    ...
    this.setData({
      ...
      isBeautyUser: app.globalData.canUseUpgrade || app.globalData.loginType === 'admin',
      hideBeautyEntry: app.globalData.canUseUpgrade && app.globalData.loginType !== 'admin',
      showBeautyQuick: app.globalData.canUseUpgrade || app.globalData.loginType === 'admin',
      canSeeUpgrade: app.globalData.canUseUpgrade || app.globalData.isPaid || app.globalData.loginType === 'admin',
      showBasicPlanEntry: (!app.globalData.canUseUpgrade && !app.globalData.isPaid) || app.globalData.loginType === 'admin',
      dayData: todayData,
      waterMl: waterMl,
      waterGoal: waterGoal,
      waterCups: waterCups,
  ...
  checkLocked: function () {
    if (this.data.isLocked) {
      this.setData({ showPayModal: true, payModalMode: 'expired' })
      return true
    }
    return false
  },
  ...
  onOpenBasicPay: function () {
    audio.playEnter()
    /* 试用期主动点击购买：弹出 unlock 模式弹窗 */
    this.setData({ showPayModal: true, payModalMode: 'unlock' })
  },
  ...
  // ===== 我的 - 打开子页面 =====
  onOpenMySub: function(e) {
    var key = e.currentTarget.dataset.key;
    var title = e.currentTarget.dataset.title;
    wx.navigateTo({
      url: '/pages/my-detail/my-detail?key=' + key + '&title=' + encodeURIComponent(title)
    });
  },
      ...
      if (scData && scData.dailyRecords) {
        for (var sci = 0; sci < scData.dailyRecords.length; sci++) {
          ...
          if (rec.date === todayStr) {
            ...
            var skinStatusLabelMap = {hydrated:'水润',dry:'干燥',oily:'泛油',acne:'痘痘',combo:'外油内干',closedComedo:'闭口',acneMark:'痘印',blackhead:'黑头',sting:'刺痛',largePores:'毛孔粗大',itchy:'发痒',fineLines:'干纹',dull:'暗沉',edema:'水肿',sensitive:'敏感泛红'}; var skinStatusLabels = rec.skinStatus.map(function(s) { return skinStatusLabelMap[s] || s; }); if (rec.skinStatus && rec.skinStatus.length > 0) posterSkincare.push({label:'皮肤状况', val:skinStatusLabels.join('、'), done:true});
            if (rec.skinStatusCustom) posterSkincare.push({label:'状况备注', val:rec.skinStatusCustom, done:true});
            if (rec.bodyCare && rec.bodyCare.length > 0) {
              var bodyCareLabelMap = {faceCare:'面部护理',oralCare:'口腔护理',neckCare:'颈部护理',lipsCare:'护唇',bodyCare:'身体护理',handCare:'手部护理',footCare:'脚部护理',eyeCare:'护眼',chestCare:'胸部护理',intimateCare:'私处护理',hairCare:'头发护理',nailCare:'美甲'};
              var bodyCareLabels = rec.bodyCare.map(function(bc) { return bodyCareLabelMap[bc] || bc; });
              posterSkincare.push({label:'身体护理', val:bodyCareLabels.join('、'), done:true});
            }
            ...
            if (rec.customCheck) posterSkincare.push({label:'自定义打卡', val:rec.customCheck, done:true});
      ...
      if (mdData && mdData.dailyRecords) {
        for (var mdi = 0; mdi < mdData.dailyRecords.length; mdi++) {
          ...
          if (mrec.date === todayStr) {
            ...
            if (mrec.discipline) {
              var discLabelMap = {disc_earlyRise:'早起',disc_water:'喝水',disc_meditation:'冥想',disc_podcast:'听播客',disc_read:'阅读',disc_news:'今日新闻',disc_study:'学习打卡',disc_newSkill:'学习新技能',disc_english:'学英语',disc_journal:'写日记',disc_gratitude:'感恩日记',disc_plan:'写今日规划',disc_cook:'做饭',disc_cleanRoom:'打扫房间',disc_exercise:'运动',disc_declutter:'断舍离',disc_lessScreen:'少玩电子产品',disc_skincare:'护肤',disc_earlySleep:'早睡',tidyRoom:'收拾房间',declutter:'断舍离',read:'阅读',exercise:'运动',cook:'做饭',noPhone:'减少手机',earlySleep:'早睡',gratitude:'感恩记录',meditation:'冥想',skincare:'护肤',water:'喝水',journal:'写日记'};
              var discItems = [];
              for (var dk in mrec.discipline) { if (mrec.discipline[dk]) discItems.push(discLabelMap[dk] || dk); }
