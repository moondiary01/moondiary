...
// 重定向：微信用户绑定手机号后，数据写到手机号路径
function resolveStorageKey(userKey) {
  // 微信用户查本地缓存的绑定手机号
  if (userKey && userKey.indexOf('wx_') === 0) {
    var bound = getLocal('wx_bound_phone');
...
function saveState(userKey, state) {
  // 重定向到真实手机号（如有绑定）
  userKey = resolveStorageKey(userKey);
  // 记录更新时间戳
...
// ===== 微信用户 ↔ 手机号绑定 =====
function loadWxPhoneBindings(callback) {
  loadFromCloud('_wx_phone_bindings', callback);
}
function saveWxPhoneBindings(bindings) {
  saveToCloud('_wx_phone_bindings', bindings);
}
...
module.exports = {
  ...
  loadWxPhoneBindings, saveWxPhoneBindings,
